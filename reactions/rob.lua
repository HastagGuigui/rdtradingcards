...
function reaction.run(message, interaction, data, response)
  ...
  if response == "yes" then
    ...
    if data.itemtype == "consumable" then
      ...
      local robweight = 3
      if robchance >= math.max((7.5*(rob_weight*1.5)*(uj.robheat+1)*(1+robmustache*0.25)*(1-doubleedge*0.15))*100, 90) then
        robsucceed = true
      end
