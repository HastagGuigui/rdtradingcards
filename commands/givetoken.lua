...
function command.run(message, mt)
	...
	if not (#mt == 1 or #mt == 2) and not (mt.user ~= nil and mt.amount ~= nil) then
		message:reply(lang.no_arguments)
		return
	end

    local user_request = mt[1] or (mt.user and mt.user.id)
	local amnt_request = mt[2] or mt.amount
	local uj2f = ""
	local numtokens = 1
	if user_request then
		uj2f = usernametoid(user_request)
		if not uj2f then
			message:reply(formatstring(lang.no_user, { user_request }))
			return
		end

		if tonumber(amnt_request) then
			if tonumber(amnt_request) > 1 then numtokens = math.floor(tonumber(amnt_request)) end
		end
	elseif mt.user then
		uj2f = mt.user.id
		if mt.amount then
