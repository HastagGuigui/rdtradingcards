local command = {
    name = "smell",
    description = "Rev up those sinuses, it's time to smell anything and everything!",
    options = {
    	{name = "thing", type = 3, required = true, description = "What you want to smell"}
	}
}
function command.run(message, mt)
	print(message._author.name .. " did !smell")
	if mt.thing or #mt == 1 then
		local uj = db.get_user(message._author.id)
		local lang = dpf.loadjson("langs/" .. uj.lang .. "/smell.json")
		local wj = dpf.loadjson("savedata/worldsave.json", defaultworldsave)
		local request = string.lower(mt[1] or mt.thing)
		print(request)
		...
		if uj.lang ~= "en" then
			for en_name, tr_name in pairs(lang.request) do
				if tr_name == request then
					request = en_name
					break
				end
			end
		...
		if curfilename ~= nil then
			if uj.inventory[curfilename] or uj.storage[curfilename] or shophas(curfilename) then
				...
				if uj.lang == "ko" then
					...
					local desc = { "그 냄새는 ", "", "사실 당신은 맛을 봤다. 카드의 맛은 ", "외부의 목소리가 당신의 머릿속에 들어온다. ", "", "당신은 갑자기 ",
						"그 카드는 " }
					local desc2 = { "을(를) 떠올리게 한다.", " 냄새가 난다.", " 같다.", "에 대해 말하고 있다.", "에 대한 그리운 기억이 당신을 스쳐 지나간다.",
						"에 대한 극심한 갈망을 느낀다.", "을(를) 떠올리게 하는 강렬한 향취를 내뿜는다." }
					local cdesc = math.random(1, #desc)
					local trfstring = "당신은 **" ..
					cdb[curfilename].name ..
					"** 카드를 " ..
					take[ctake] ..
